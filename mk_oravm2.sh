...
if [[ -z "$groupname" ]]; then
	echo "What is the name for the resource group you want to add this VM to? (the resource group must already exist) "
...
az vm image list --offer Oracle-database --all --publisher Oracle --output table >db.lst
